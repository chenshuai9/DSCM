function [outdp] = getDeltaPFANR(rowInit, colInit, rows, cols, config, p ,fData, gData)
    ...
    for row = 1:size(rows,2)
        for col = 1:size(cols, 2)  
                ...
                dxdp = [1/p.u_x;deltax;deltay;1/p.v_x;0;0];
                dydp = [1/p.u_y;0;0;1/p.v_y;deltax;deltay];
                %dgdp£∫6*1
                for i=1:1:6
                    gdgdp(i,1) = gdgdp(i,1)+gData.values(row, col)*(g_x(row,col)*dxdp(i,1)+g_y(row,col)*dydp(i,1));
                    fdgdp(i,1) = fdgdp(i,1)+fData.values(row, col)*(g_x(row,col)*dxdp(i,1)+g_y(row,col)*dydp(i,1));
                end
